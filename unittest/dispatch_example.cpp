...
#include "base/vcl/vectorclass.h"
...
int test_vcl(int argc, char* argv[]) 
{
    ...
    int b[4] = { 1, 2, 3, 4 };
    float d[4] = {0};
    __m128 scale = _mm_set_ps( 2, 4, 8, 16 );
    __m128i x = _mm_loadu_si128( ( __m128i const* )b );
    __m128 dx = _mm_cvtepi32_ps( x );
    dx = _mm_mul_ps( dx, scale );
    _mm_storeu_ps( d, dx);
    return 0;
}
