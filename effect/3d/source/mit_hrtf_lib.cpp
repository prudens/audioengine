...
static unsigned int mit_hrtf_get_impl(int* pAzimuth, int* pElevation, unsigned int samplerate, unsigned int diffused, const short** psLeft, const short** psRight)
{
...
unsigned int mit_hrtf_get( int* pAzimuth, int* pElevation, unsigned int samplerate, unsigned int diffused, short* psLeft, short* psRight )
{
    const short*pLeft;
    const short *pRight;
    unsigned int nTaps = mit_hrtf_get_impl( pAzimuth, pElevation, samplerate, diffused, &pLeft, &pRight );
    if (nTaps)
    {
        memcpy( psLeft, pLeft, nTaps*sizeof( short ) );
        memcpy( psRight, pRight, nTaps*sizeof( short ) );
    }
    return nTaps;
}
...
unsigned int mit_hrtf_get( int* pAzimuth, int* pElevation, unsigned int samplerate, unsigned int diffused, float* psLeft, float* psRight )
{
    ...
    if ( nTaps )
    {
        static const float kMaxInt16Inverse = 1.f / std::numeric_limits<short>::max();
        static const float kMinInt16Inverse = 1.f / std::numeric_limits<short>::min();
        for ( unsigned int i = 0; i < nTaps; i++ )
        {
            psLeft[i] = pLeft[i] * ( pLeft[i] > 0 ? kMaxInt16Inverse : -kMinInt16Inverse );
            psRight[i] = pRight[i] * ( pRight[i] > 0 ? kMaxInt16Inverse : -kMinInt16Inverse );
        }
    }
    return nTaps;
}
