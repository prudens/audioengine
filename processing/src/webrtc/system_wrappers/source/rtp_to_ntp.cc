...
bool RtpToNtpMs(int64_t rtp_timestamp,
  ...
  if ( !CompensateForWrapAround( static_cast<uint32_t>( rtcp_timestamp_new),
                               static_cast<uint32_t>(rtcp_timestamp_old),
                               &rtcp_timestamp_new)) {
    return false;
  }
  double freq_khz;
  if (!CalculateFrequency(rtcp_ntp_ms_new,
                          static_cast<uint32_t>( rtcp_timestamp_new),
                          static_cast<uint32_t>( rtcp_timestamp_old ),
                          (uint32_t)rtcp_timestamp_old,
                          &freq_khz)) {
    return false;
  }
  double offset = rtcp_timestamp_new - freq_khz * rtcp_ntp_ms_new;
  int64_t rtp_timestamp_unwrapped;
  if ( !CompensateForWrapAround( static_cast<uint32_t>( rtp_timestamp), static_cast<uint32_t>( rtcp_timestamp_old ),
  ...
  *rtp_timestamp_in_ms = static_cast<int64_t>(rtp_timestamp_ntp_ms);
  return true;
}
