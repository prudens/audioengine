...
#include "webrtc/base/rtc_event_log.h"
    ...
    bool over_rtx;
    {
      ...
      if (rtx_ == kRtxOff) {
        ...
        if (last_timestamp_time_ms_ > 0) {
          timestamp +=
              static_cast<uint32_t>((clock_->TimeInMilliseconds() - last_timestamp_time_ms_) * 90);
          capture_time_ms +=
...
void RTPSender::OnReceivedNACK(const std::list<uint16_t>& nack_sequence_numbers,
  ...
  if (!ProcessNACKBitRate((uint32_t)now)) {
...
void RTPSender::UpdateNACKBitRate(uint32_t bytes, int64_t now) {
  ...
  nack_byte_count_times_[0] = (uint32_t)now;
}
...
void RTPSender::UpdateDelayStatistics(int64_t capture_time_ms, int64_t now_ms) {
  ...
  }
  {
    ...
    send_delays_[now_ms] = static_cast<uint32_t>(now_ms - capture_time_ms);
...
void RTPSender::UpdateTransmissionTimeOffset(uint8_t* rtp_packet,
  ...
  ByteWriter<int32_t, 3>::WriteBigEndian(rtp_packet + offset + 1,
                                         (uint32_t)time_diff_ms * 90);  // RTP timestamp.
}
