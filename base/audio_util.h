...
//{
	...
	// Down mixes an interleaved multichannel signal to a single channel by averaging
	// all channels.
	template <typename T, typename Intermediate>
	void DownmixInterleavedToMonoImpl(const T* interleaved,
		...
		T* deinterleaved)
	{
		if (!interleaved || !deinterleaved || num_channels == 0 || num_frames == 0)
		{
			throw std::invalid_argument("Invalid DownmixInterleavedToMonoImpl argument");
		}
		...
		while (interleaved < end)
		{
			...
			*deinterleaved++ = static_cast<T>(value / num_channels);
		}
	}
