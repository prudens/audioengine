...
public:
	...
	virtual std::error_code SendTo(std::string ip, int port, const void* data, size_t length) = 0;
	virtual std::error_code SendTo( const void* data, size_t length) = 0;
	virtual void AsyncSendTo(std::string ip, int port, const void* data, size_t length, WriteHandler handler) = 0;
